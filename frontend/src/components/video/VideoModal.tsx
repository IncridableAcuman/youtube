...
// Formaning boshlang'ich holatini qaytaruvchi helper
const getInitialState = (videoToEdit?: Video | null) => ({
  title: videoToEdit?.title || "",
  description: videoToEdit?.description || "",
  youtubeUrl: videoToEdit?.youtubeUrl || "",
  duration: videoToEdit?.duration || "10:00",
  category: videoToEdit?.category || (CATEGORIES[0] as string),
...
export function VideoModal({ isOpen, onClose, videoToEdit }: VideoModalProps) {
  ...
  // Prop o'zgarganda state-ni kuzatish uchun maxsus o'zgaruvchilar
  const [prevVideoToEdit, setPrevVideoToEdit] = useState(videoToEdit);
  const [prevIsOpen, setPrevIsOpen] = useState(isOpen);
  const [formData, setFormData] = useState(() => getInitialState(videoToEdit));

  // React-ning "Adjusting state while rendering" formati (useEffect-siz synchronous sinxronlash)
  if (videoToEdit !== prevVideoToEdit || isOpen !== prevIsOpen) {
    setPrevVideoToEdit(videoToEdit);
    setPrevIsOpen(isOpen);
    setFormData(getInitialState(videoToEdit));
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = videoToEdit
      ? await editVideo(videoToEdit.id, payload)
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
            ...
            {youtubeId && (
              <div className="mt-2 flex items-center gap-3 p-2.5 rounded-xl bg-zinc-950 border border-zinc-800">
                ...
                <div className="text-xs text-zinc-400 overflow-hidden">
                  ...
                  <p className="text-[11px] text-zinc-500 truncate max-w-60">
                    {formData.youtubeUrl}
                  </p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label className="flex items-center gap-1.5 text-xs font-medium text-zinc-300">
                <Clock className="w-3.5 h-3.5 text-zinc-400" /> Davomiyligi
                (MM:SS)
              </label>
              <input
                type="text"
          ...
          <div className="space-y-1.5">
            <label className="flex items-center gap-1.5 text-xs font-medium text-zinc-300">
              <Tag className="w-3.5 h-3.5 text-zinc-400" /> Teglar (vergul bilan
              ajrating)
            </label>
            <input
              type="text"
